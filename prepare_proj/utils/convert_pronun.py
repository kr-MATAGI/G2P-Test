...
class ConvertPronun():
    ...
    def conv_ipa2kor_unit_sent(self, origin: str, ipa: str):
        mapping = pd.read_csv("../data/kr/dictionary.txt", header=None, delimiter="\t")
        self.dic = {}
        ...
        conv_ipa = re.sub(r"[ ːˈˌ]", "", ipa)

        # ipa -> 발음대로
        for ph, gr in self.dic.items():
            conv_ipa = conv_ipa.replace(ph, gr)
        ...
        if exp in conv_ipa:
            # 원래 단어 자모로 분해
            origin_jamo = split_syllables(origin)
            for j in range(len(origin_jamo)):
                if 'ㅚ' in origin_jamo[j]:
                    origin_we_list.append('ㅚ')
                elif 'ㅞ' in origin_jamo[j]:
                    ...
                    continue
            # e -> ㅚ or ㅞ
            print(origin_we_list)
            for we in origin_we_list:
                conv_ipa = conv_ipa.replace(exp, we, 1)
        ...
        result_list = []
        word_list = list(conv_ipa)  # 한 글자씩 분해
        index = 0
        for i in range(len(word_list)):
            # 첫 글자가 모음일 경우
            if i == 0 and conv_ipa[i] in vowel:
                word_list.insert(0, "ㅇ")
                index += 1
            else:
                # 전 글자가 이응으로 시작하거나 모음만 있는 경우 이응 추가
                if (conv_ipa[i] in vowel and conv_ipa[i - 1] == "ㅇ") or (conv_ipa[i] in vowel and conv_ipa[i - 1] in vowel):
                    word_list.insert(index, "ㅇ")
                    index += 1
            index += 1
        result = "".join(word_list)
...
if __name__ == "__main__":
    ...
    pronun.conv_ipa2kor_unit_sent(origin="이제", ipa="iʣe")